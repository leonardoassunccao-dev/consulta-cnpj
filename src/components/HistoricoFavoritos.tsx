        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={`px-4 py-2 text-xs md:text-sm font-semibold transition relative ${
              activeTab === 'history' ? 'text-white font-medium' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
            📋 Histórico Recente ({history.length})
            {activeTab === 'history' && (
              <span className="absolute bottom-[-11px] left-0 right-0 h-[1.5px] bg-zinc-400"></span>
            )}
          </button>
          <button
            onClick={() => setActiveTab('favorites')}
            className={`px-4 py-2 text-xs md:text-sm font-semibold transition relative ${
              activeTab === 'favorites' ? 'text-white font-medium' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
