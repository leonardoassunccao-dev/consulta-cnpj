...
import { sanitizeSearchTerm, searchCompanyDirectory, SUGGESTION_MAX_RESULTS } from '../_lib/companySearch.js';
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (!allowRequest(`suggestions:${requestIdentity(req.headers)}`, 30)) return res.status(429).json({ error: 'Muitas solicitações.' });
